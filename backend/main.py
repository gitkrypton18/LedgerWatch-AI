...
import pandas as pd
import shap
from fastapi import (Depends, FastAPI, File, HTTPException, Query, Security,
                     UploadFile)
...
from src.schemas import (BatchPredictionResponse, HealthResponse,
                         OCRExtraction, PredictionResult, TransactionCreate,
                         TransactionRead)
                ...
                for start_idx in range(0, total_rows, batch_size):
                    ...
                    for _, row in batch.iterrows():
                        tx = DBTransaction(
                            ...
                            isFlaggedFraud=int(row.get("isFlaggedFraud", 0)) if "isFlaggedFraud" in row else None,
                            risk_score=50,
                            risk_band="Medium",
                            is_anomaly=False,
                        )
                    ...
                    db.commit()
                    logger.info(f"  Seeded rows {start_idx + 1}–{end_idx} / {total_rows}")
