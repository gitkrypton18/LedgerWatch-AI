...
## Days 0–3: Scaffold to Feature Engineering
...
**Current Status:** Day 3 Complete, ready for Day 4 (train.py)
...
6. [Key Findings & Interview Talking Points](#6-key-findings--interview-talking-points)
7. [Module Dependency Chain](#7-module-dependency-chain)
8. [Next Steps: Day 4 Preview](#8-next-steps-day-4-preview)
...
- Created `notebooks/day3_feature_engineering.ipynb` with 15 cells
...
|-----------|---------------|
| **No label leakage** | `isFraud` NEVER used during feature engineering; runtime assertion guards against accidental inclusion |
| **No future leakage** | `freq_orig/dest` use EXPANDING window (cumcount) — only counts previous transactions |
| **Production-ready** | All features computable on single new transaction without retraining |
| **EDA-driven** | Every feature directly traces back to a Day 2 insight |
| **Modular** | 6 separate engineering functions, each testable in isolation |

### 5.3 Feature Inventory (24 Total)

| # | Feature | Source Insight | Type | Function |
|---|---------|---------------|------|----------|
| 1 | `amount_log` | Amount skewness > 4 | Continuous | `engineer_amount_features` |
| 2 | `is_round_amount` | Round numbers slightly suspicious | Binary | `engineer_amount_features` |
| 3 | `amount_to_balance_ratio` | Sender liquidity stress | Continuous | `engineer_amount_features` |
| 4 | `balance_diff_orig` | Inverted anomaly finding | Continuous | `engineer_balance_features` |
| 5 | `balance_diff_dest` | Same for destination | Continuous | `engineer_balance_features` |
| 6 | `balance_change_orig` | Signed direction | Continuous | `engineer_balance_features` |
| 7 | `balance_change_dest` | Signed direction | Continuous | `engineer_balance_features` |
| 8 | `zero_balance_orig` | Sender emptied account | Binary | `engineer_balance_features` |
| 9 | `zero_balance_dest` | Recipient started at zero | Binary | `engineer_balance_features` |
| 10 | `hour_of_step` | Hour extraction (0-23) | Cyclical | `engineer_temporal_features` |
| 11 | `hour_of_step_sin` | Cyclical encoding (sin) | Continuous | `engineer_temporal_features` |
| 12 | `hour_of_step_cos` | Cyclical encoding (cos) | Continuous | `engineer_temporal_features` |
| 13 | `type_encoded` | Fraud risk ordinal | Ordinal | `engineer_categorical_features` |
| 14 | `type_CASH_IN` | One-hot dummy | Binary | `engineer_categorical_features` |
| 15 | `type_CASH_OUT` | One-hot dummy | Binary | `engineer_categorical_features` |
| 16 | `type_DEBIT` | One-hot dummy | Binary | `engineer_categorical_features` |
| 17 | `type_PAYMENT` | One-hot dummy | Binary | `engineer_categorical_features` |
| 18 | `type_TRANSFER` | One-hot dummy | Binary | `engineer_categorical_features` |
| 19 | `freq_orig` | Expanding count per sender | Count | `engineer_frequency_features` |
| 20 | `freq_dest` | Expanding count per recipient | Count | `engineer_frequency_features` |
| 21 | `is_new_orig` | First transaction for sender | Binary | `engineer_frequency_features` |
| 22 | `is_new_dest` | First transaction for recipient | Binary | `engineer_frequency_features` |
| 23 | `is_merchant_orig` | Sender is merchant | Binary | `engineer_merchant_features` |
| 24 | `is_merchant_dest` | Recipient is merchant | Binary | `engineer_merchant_features` |

### 5.4 Key Design Decisions

| Decision | Rationale |
|----------|-----------|
| **Expanding window for freq features** | `cumcount()` counts only PREVIOUS transactions per account — prevents data leakage and works on new transactions in production |
| **Log1p not log** | Handles amount=0 safely (though PaySim min is ~0.32) |
| **Cyclical sin/cos for hour** | Preserves circular nature (hour 23 ≈ hour 0); linear encoding would make midnight far from 11 PM |
| **Balance diff as continuous** | EDA showed inverted finding — magnitude matters, not binary flag. Isolation Forest learns the nuanced pattern |
| **Type risk map (4→0)** | Encodes domain knowledge: TRANSFER riskiest (0.66% fraud), DEBIT safest (0% fraud) |
| **99.9th percentile cap on ratio** | Prevents extreme outliers from breaking models; cap learned from training data, applied consistently |
| **isFraud assertion in pipeline** | Runtime guard against accidental label leakage — fails fast if `isFraud` ever enters feature columns |

### 5.5 Notebook Structure (15 Cells)

```
notebooks/day3_feature_engineering.ipynb
├── Cell 1:   Imports & Setup
├── Cell 2:   Project Paths
├── Cell 3:   Load Data (100K sample or synthetic fallback)
├── Cell 4:   Quick EDA Recap — Verify Day 2 findings
├── Cell 5:   Feature 1-3: Amount Features
├── Cell 6:   Feature 4-9: Balance Features
├── Cell 7:   Feature 10-12: Temporal Features
├── Cell 8:   Feature 13-18: Categorical Features
├── Cell 9:   Feature 19-22: Frequency Features (expanding window)
├── Cell 10:  Feature 23-24: Merchant Features
├── Cell 11:  Feature Summary & Validation
├── Cell 12:  Correlation Analysis (features vs isFraud)
├── Cell 13:  Feature Distributions by Fraud Class
├── Cell 14:  Save features.csv
└── Cell 15:  Generate Production src/features.py
```

### 5.6 Production Module: `src/features.py`

**Architecture:**
```
src/features.py
├── engineer_amount_features()      # amount_log, is_round_amount, amount_to_balance_ratio
├── engineer_balance_features()     # balance_diff_orig/dest, balance_change_orig/dest, zero_balance_orig/dest
├── engineer_temporal_features()    # hour_of_step, hour_of_step_sin/cos
├── engineer_categorical_features() # type_encoded, one-hot dummies
├── engineer_frequency_features()   # freq_orig/dest, is_new_orig/dest (EXPANDING — no leakage)
├── engineer_merchant_features()    # is_merchant_orig/dest
└── engineer_all_features()         # Orchestrator: load → engineer → validate → save
```

**Validation Checks (in `engineer_all_features`):**
- ✅ No NaNs in engineered features
- ✅ No infinities in engineered features
- ✅ `isFraud` NOT in feature columns (assertion — fails fast on leakage)
...
df = engineer_all_features()  # Uses default paths from config
```
...
| | Path | Rows | Columns |
|---|------|------|---------|
| **Input** | `data/processed/cleaned.csv` | 6,362,620 | 11 |
| **Output** | `data/processed/features.csv` | 6,362,620 | 35 (11 original + 24 engineered) |
...
## 6. Key Findings & Interview Talking Points
...
### 6.4 Day 3 Talking Points

**8. Feature Engineering Discipline:**
> "I engineered 24 features from 11 original columns, and every single one traces back to an EDA insight. I don't create features blindly — amount_log came from skewness > 4, zero_balance_orig came from the inverted balance finding, and freq_orig uses an expanding window to prevent data leakage."
...
> "Every feature is computable on a single new transaction without retraining. The pipeline is modular — 6 separate engineering functions, each testable in isolation. And I have runtime assertions that fail fast if labels ever leak into features."

**11. Cyclical Encoding:**
...
Day 3: Feature Engineering ✅
    ...
    └── notebooks/day3_feature_engineering.ipynb ✅ (15 cells)

Day 4: Model Training (NEXT)
...
## 8. Next Steps: Day 4 Preview

### 8.1 Goal
Create `src/train.py` — train Isolation Forest on engineered features, save model as `isolation_forest_v1.0.0.joblib`

### 8.2 What Will Be Built

| Component | Description |
|-----------|-------------|
| `src/train.py` | Training pipeline: load features → split → fit Isolation Forest → save model |
| `notebooks/day4_model_training.ipynb` | Verification notebook with training curves, convergence checks |

### 8.3 Key Design Decisions (Locked)

| Decision | Value | Rationale |
|----------|-------|-----------|
| Model | Isolation Forest | Unsupervised, fast, SHAP-compatible |
| Contamination | 0.01 (from `.env`) | Matches expected fraud rate; adjustable |
| Features used | All 24 engineered | Isolation Forest handles high dimensionality |
| Labels during training | NONE | Honest unsupervised approach |
| Labels during validation | `isFraud` column | Compute precision/recall/ROC-AUC post-hoc |
| Output format | joblib | Fast loading, preserves sklearn internals |

### 8.4 Validation Plan

- Train on 80% of data (random split, stratify by type if needed)
- Validate on 20% — compute unsupervised metrics (anomaly score distribution)
- Cross-reference with `isFraud` labels: precision, recall, F1, ROC-AUC
- Compare against baseline: random classifier, rule-based (balance anomaly flag)
...
| Features CSV | `F:\ML PROJECT\LedgerWatch-AI\LedgerWatch-AI\data\processedeatures.csv` |
...
| Feature Eng. Notebook | `F:\ML PROJECT\LedgerWatch-AI\LedgerWatch-AI
otebooks\day3_feature_engineering.ipynb` |
...
| Features | `F:\ML PROJECT\LedgerWatch-AI\LedgerWatch-AI\srceatures.py` |
...
# Confirm features module works
python -c "from src.features import engineer_all_features; print('Features module OK')"
# Expected: Features module OK

# Run full feature pipeline
python src/features.py
# Expected: Logs showing load -> engineer -> validate -> save
...
*End of Documentation — Days 0-3 Complete*  
*Next: Day 4 — Model Training (`src/train.py`)*
