...
## Days 0–4: Scaffold to Model Training
...
**Current Status:** Day 4 Complete, ready for Day 5 (Evaluation)
...
7. [Day 5: Evaluation](#7-day-5-evaluation) *(Next)*
...
| `src/data_ingest.py` | ETL pipeline | Row-by-row Pydantic validation + bulk SQL insert |
...
### 4.4 Output
- `data/processed/cleaned.csv` — 6,362,620 rows × 11 columns, 498.9 MB
...
| Output | `data/processed/features.csv` | 6,362,620 | 35 (11 + 24) |
...
## 7. Day 5: Evaluation *(Next)*

### 7.1 Goal
Create `src/evaluate.py` — comprehensive model evaluation with curves, thresholds, and baseline comparison.

### 7.2 Planned Components

| Component | Description |
|-----------|-------------|
| `src/evaluate.py` | Evaluation pipeline: PR curve, ROC curve, confusion matrix, threshold tuning |
| `notebooks/day5_evaluation.ipynb` | Interactive plots and threshold analysis |
| `tests/test_evaluate.py` | Unit tests for metrics computation |

### 7.3 Key Design Decisions (Locked)

| Decision | Value | Rationale |
|----------|-------|-----------|
| Metrics | Precision, Recall, F1, ROC-AUC, PR-AUC | Standard + PR-AUC for imbalanced data |
| Baseline | Random classifier + Rule-based (balance anomaly) | Prove model beats simple alternatives |
| Threshold tuning | F1-optimized + Precision-optimized | Different production needs |
| Output | JSON metrics + PNG plots | Reproducible, version-controllable |
...
| `/batch-predict` | POST | Bulk prediction from CSV |
| `/ocr` | POST | Invoice PDF upload + parse |
| `/transactions` | GET | Query transaction history |
...
| Page | Description |
|------|-------------|
| Upload | CSV/PDF upload + preview |
| Predictions | Risk scores table + filters |
| Explain | SHAP waterfall plots per transaction |
| Analytics | Fraud trends, feature distributions |
...
| `tests/test_train.py` | Model training ✅ |
...
*End of Days 0-4 Documentation*  
*Next: Day 5 — Evaluation (`src/evaluate.py`)*
